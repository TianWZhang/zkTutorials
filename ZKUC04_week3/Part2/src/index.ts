...
const encrypt = async (
  ...
  //generate the IV, use Mimc7 to hash the shared key with the IV, then encrypt the plain text
...
const decrypt = async (
  ...
  //use Mimc7 to hash the shared key with the IV, then descrypt the ciphertext
  const mimc7 = await buildMimc7();

  const plaintext: Plaintext = ciphertext.data.map(
    (e: bigint, i: number): bigint => {
      return (
        e - buf2Bigint(mimc7.hash(sharedKey, BigInt(ciphertext.iv) + BigInt(i)))
      );
    },
  );
